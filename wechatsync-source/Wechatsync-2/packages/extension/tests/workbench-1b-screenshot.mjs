...
const capabilities = {
  ...
  runtime: {
    serviceVersion: '0.6.2-stage7-caption-cleanup',
    protocol: { name: 'yizao-local-service', version: 2 },
    acceptanceBuildId: 'stage7-caption-cleanup-v3.8',
    requiredExtensionBuildId: 'stage7-caption-cleanup-v3.8',
  },
  actions: {
    upload: '真实上传/保存草稿',
    publish: '公开发布',
  ...
  platforms: [
    ...
    { id: 'xiaohongshu', name: '小红书', aliases: ['小红书'], group: '主流平台', status: 'guarded-draft-unverified', workflow: 'guarded-draft', currentActions: ['只读扫描', '受保护单篇长文草稿'], plannedActions: ['真实验收'], evidence: ['自动测试'], risks: ['真实账号未验收'] },
  ],
}
...
try {
  ...
  await page.route('http://127.0.0.1:8788/**', async (route) => {
    ...
    if (req.url().endsWith('/api/health')) return route.fulfill({ json: { ok: true, name: 'yizao-sync-service', version: healthMismatch ? 'old-service' : '0.6.2-stage7-caption-cleanup', protocol: { name: 'yizao-local-service', version: 2 }, build: { packageVersion: 38, id: 'stage7-caption-cleanup-v3.8', extensionBuildId: 'stage7-caption-cleanup-v3.8' } } })
  ...
  await page.getByRole('button', { name: '未归档文章', exact: true }).click()
  ...
  assert.equal(await page.locator('.source-group').count(), 4, '文章库只显示知乎/搜狐/头条/网易，官网和百家号发布入口已移除')
  assert.equal(await page.locator('.pkg[data-cap="publish-preview"]').count(), 0, '官网和百家号不得再出现发布流程入口')
  assert.equal(await page.getByRole('button', { name: /易造新品发布通稿/ }).count(), 0, '官网文章发布卡不再显示')
  assert.equal(await page.getByRole('button', { name: /百家号图文/ }).count(), 0, '百家号文章发布卡不再显示')
  assert.equal(await page.locator('.pkg[data-cap="guarded-draft"]').count(), 4, '知乎/搜狐/头条/网易 = 受保护单篇草稿')
  assert.equal(await page.locator('.pkg[data-cap="draft-preview"]').count(), 0, '网易不再使用草稿流程预览')
  assert.equal(await page.locator('.pkg[data-cap="not-ready"]').count(), 0, '当前截图夹具不含待适配包')
  ...
  assert.equal(await page.locator('.pkg').count(), 4, '查看全部应清除组合筛选，但不恢复已移除的官网和百家号入口')
  await shot('workbench-1b-library.png')

  await page.getByRole('button', { name: '平台与账号' }).click()
  await page.getByText('真实动作未启用').waitFor()
  assert.equal(await page.locator('.cap-card[data-status="simulation-ready"]').count(), 0, '官网和百家号模拟发布能力不再显示')
  assert.equal(await page.locator('.cap-card').filter({ hasText: '官方网站' }).count(), 0, '平台页不再显示官方网站发布入口')
  assert.equal(await page.locator('.cap-card').filter({ hasText: '百家号' }).count(), 0, '平台页不再显示百家号发布入口')
  ...
  await page.getByRole('button', { name: '未归档文章' }).click()
  await page.locator('.pkg-wrap').filter({ hasText: '智能雷暴仪预警应用' }).getByRole('button', { name: '查看详情' }).click()
  await page.getByRole('button', { name: '查看检查详情' }).click()
  await page.getByText('自动检查已通过。').waitFor()
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎', exact: true }).isEnabled(), true, '主按钮无需用户预先执行检查或勾选确认')
  ...
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎', exact: true }).isDisabled(), true, '服务/扩展版本不匹配时真实草稿按钮必须阻止')
  healthMismatch = false
  await page.getByRole('button', { name: '重新检查' }).click()
  await page.getByText('自动检查已通过。').waitFor()
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎', exact: true }).isEnabled(), true, '主按钮本身构成当次明确草稿授权，不追加第二个弹窗')
  await shot('workbench-3-zhihu-one-click.png')
  await page.getByRole('button', { name: '生成验收材料（只读）' }).click()
  ...
  // 回文章库复核官网和百家号发布入口已移除，历史任务也不再显示。
  await page.getByRole('button', { name: '未归档文章' }).click()
  assert.equal(await page.getByText('发布流程预览', { exact: false }).count(), 0, '工作台不再渲染官网/百家号发布流程预览')
  await shot('workbench-1b-publish-preview.png')
  await page.getByRole('button', { name: '任务中心', exact: false }).click()
  assert.equal(await page.getByText('易造新品发布通稿（模拟）').count(), 0, '官网历史模拟发布任务不再显示')
  ...
  await page.getByPlaceholder('正式链接（模拟，可留空）').first().fill('https://zhuanlan.zhihu.com/p/demo')
  ...
  assert.equal(await page.locator('main').innerText().then((t) => /未移动[、，]?(?:未)?复制(?:或|、)(?:未)?删除真实文件/.test(t)), true, '归档确认必须说明未移动、复制或删除真实文件')
  await shot('workbench-2f-archive-confirm.png')
  await shot('workbench-2g-task-stepper.png')

  assert.deepEqual(errors, [])
  console.log('WORKBENCH SHOTS PASS: official/Baijiahao publishing entries and historical simulation tasks are hidden; guarded drafts, safety gates, read-only acceptance material, task center and simulation state controls remain covered with mock data.')
} finally {
  await browser?.close()
