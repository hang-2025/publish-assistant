...
try {
  ...
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎' }).isEnabled(), true, '主按钮无需用户预先执行检查或勾选确认')
  ...
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎' }).isDisabled(), true, '服务/扩展版本不匹配时真实草稿按钮必须阻止')
  healthMismatch = false
  await page.getByRole('button', { name: '重新检查' }).click()
  await page.getByText('自动检查已通过。').waitFor()
  assert.equal(await page.getByRole('button', { name: '保存草稿并打开知乎' }).isEnabled(), true, '主按钮本身构成当次明确草稿授权，不追加第二个弹窗')
