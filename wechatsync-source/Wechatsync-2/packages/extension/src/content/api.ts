...
function isTrustedApiEvent(evt: MessageEvent): boolean {
  ...
  if (TRUSTED_API_ORIGINS.includes(evt.origin)) return true
  return import.meta.env.DEV && ['http://localhost:8080', 'http://127.0.0.1:8080'].includes(evt.origin)
}
...
window.addEventListener('message', async (evt) => {
  try {
    if (!isTrustedApiEvent(evt)) return
    const action = JSON.parse(evt.data);
    if (!action.method) return;

    // getAccounts - 仅可信控制台可读取已登录平台
