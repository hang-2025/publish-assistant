    ...
    <div className="flex flex-col h-[500px]">
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 py-2.5 border-b">
        ...
        <nav className="flex items-center gap-0.5">
          <button
            onClick={() => chrome.tabs.create({ url: chrome.runtime.getURL('src/workbench/index.html') })}
            className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg hover:bg-muted transition-colors"
            title="易造发布助手 · 工作台（只读原型）"
          >
            <LayoutDashboard className="w-3.5 h-3.5" />
            <span className="text-[10px] text-muted-foreground leading-none">工作台</span>
          </button>
          <button
