  ...
  protected async processImages(
    ...
    for (const { full, src, alt, type } of matches) {
      ...
      try {
        ...
        if (type === 'html') {
          // HTML 格式
          // Preserve ALT, captions-related attributes and layout from the sanitized source.
          replacement = full.replace(/src="[^"]*"/i, `src="${uploadResult.url.replace(/&/g, '&amp;').replace(/"/g, '&quot;')}"`)
          if (uploadResult.attrs) {
            for (const [key, value] of Object.entries(uploadResult.attrs)) {
              if (/^[\w-]+$/.test(key) && !/^on/i.test(key)) {
                replacement = replacement.replace(/\s*\/?>(?=$)/, ` ${key}="${String(value).replace(/&/g, '&amp;').replace(/"/g, '&quot;')}" />`)
              }
            }
          }
        } else {
        ...
        logger.error('Failed to upload article image', error)
        // Never report a successful article with a missing/unuploaded local image.
        throw new Error(`第 ${processed} 张图片上传失败，已停止保存文章：${(error as Error).message}`)
      }
