...
为避免重启或注销后出现“本地服务未启动”，每台电脑首次配置时再双击一次：

```text
yizao-sync-service/安装自动启动.cmd
```

它会为当前 Windows 用户创建登录启动快捷方式，只在后台保证本地服务运行，不会自动弹出 Chrome。需要取消时双击 `yizao-sync-service/取消自动启动.cmd`。该操作不复制令牌、文章、Chrome 登录数据或个人配置。
