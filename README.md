...
官网和百家号的模拟流程会停在“等待用户最终提交”；五个主流平台具有受保护的单篇保存草稿与回读流程，未完成各自真实账号人工验收前 capability 仍标记 `verified=false`、`saveDraft=false`。小红书当前只支持图文草稿，标题、正文和图片数量由创作中心本地草稿库回读验证；图片顺序和 HTML 锚点需人工检查。
