...
const ACCOUNT_ID = 'toutiao-current-session';
const MODE = 'toutiao-draft';
...
function safeDraftResult(result = {}) {
  ...
  if (!/^[0-9]+$/.test(postId)) throw new Error('头条号草稿回读结果缺少有效草稿 ID');
  let url;
  try { url = new URL(String(result.postUrl || '')); } catch { throw new Error('头条号草稿回读结果缺少有效草稿 URL'); }
  if (url.protocol !== 'https:' || url.hostname !== 'mp.toutiao.com'
      || url.pathname !== '/profile_v4/graphic/publish'
      || url.searchParams.get('from') !== 'edit' || url.searchParams.get('pgc_id') !== postId) {
    throw new Error('头条号草稿 URL 不受信任');
  }
  if (result.success !== true || result.draftOnly !== true || result.readBackVerified !== true || result.fidelityVerified !== true) {
    throw new Error('头条号草稿未通过保存后 HTML 内容保真校验，不能标记 draft_saved');
  }
  ...
  if (!fidelity || fidelity.fidelityVerified !== true
    ...
    throw new Error('头条号草稿保真报告缺失或必需检查未通过，不能标记 draft_saved');
  }
  ...
  for (const key of REQUIRED_FIDELITY_CHECKS) {
    ...
    if (!check || check.required !== true || check.status !== 'PASS') {
      throw new Error(`头条号草稿保真报告缺少必需 PASS：${key}，不能标记 draft_saved`);
    }
  }
  return { postId, postUrl: url.toString(), readBackVerified: true, fidelityVerified: true, fidelity, savedAt: new Date().toISOString() };
}

/** 持久化协调头条号单篇草稿任务；不接收 Cookie，也不直接访问头条号。 */
export class ToutiaoDraftService {
  ...
  async prepare({ packageId, userConfirmed }) {
    if (userConfirmed !== true) throw new Error('保存头条号草稿需要用户在当前操作中明确确认');
    ...
    const baseTaskKey = makeTaskKey({ packageId, platform: 'toutiao', accountId: ACCOUNT_ID, contentVersion: snapshot.source.contentVersion });
    ...
    if (previous && !canSafelyRetryBeforeSave(previous)) {
      return {
        started: false,
        reason: previous.status === TASK_STATUS.FAILED ? 'manual-review-required' : 'exists',
        task: previous,
      };
    }
    ...
    const created = await this.store.createTask({
      ...
      platform: 'toutiao', platformName: '头条号', accountId: ACCOUNT_ID, accountLabel: '当前 Chrome 头条号会话',
    ...
    if (previous) {
      ...
      created.task.retryReason = '前次失败发生在草稿保存请求之前；保留原审计记录后由用户重新确认重试';
    ...
    const task = await this.store.transitionStatus(created.task.taskId, TASK_STATUS.READY, '快照已锁定，等待扩展检查当前头条号登录会话');
    return { started: true, task, gate: snapshot.gate };
  }

  async begin({ taskId, snapshotId, userConfirmed }) {
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== MODE) throw new Error('找不到头条号草稿任务');
    ...
    const gate = checkRealActionGate({
      ...
      authorization: { stage: '5-toutiao-draft', userConfirmed, snapshotVerified: true },
    });
    if (!gate.allowed) throw new Error(gate.reason);
    const running = await this.store.transitionStatus(taskId, TASK_STATUS.RUNNING, '安全闸门已通过；仅允许保存头条号草稿');
    return { task: running, authorization: { action: 'saveDraft', platform: 'toutiao', snapshotId: task.snapshotId } };
  }

  async progress({ taskId, status, detail = '' }) {
    if (!PROGRESS.has(status)) throw new Error('不允许的头条号草稿进度状态');
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== MODE) throw new Error('找不到头条号草稿任务');
  ...
  async complete({ taskId, result }) {
    ...
    await this.store.transitionStatus(taskId, TASK_STATUS.DRAFT_SAVED, '头条号草稿已保存并通过 HTML 内容保真回读校验', { draftResult });
  ...
  async fail({ taskId, error, fidelityReport }) {
    ...
    return this.store.transitionStatus(taskId, TASK_STATUS.FAILED, String(error || '头条号草稿保存失败').slice(0, 500), fidelityFailure ? { fidelityFailure } : {});
  }
}
