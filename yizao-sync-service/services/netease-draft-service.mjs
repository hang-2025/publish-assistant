...
const ACCOUNT_ID = 'netease-current-session';
const MODE = 'netease-draft';
...
const REQUIRED_FIDELITY_CHECKS = new Set([
  ...
  'caption-equals-html-alt', 'trusted-draft-url', 'draft-only', 'read-back-verified', 'guardian-token',
]);
...
function snapshotRecord(snapshot) {
  return {
    ...
    target: 'netease', createdAt: snapshot.createdAt,
  };
}
...
function sanitizeFidelityReport(report) {
  ...
  return {
    ...
    checks: report.checks.slice(0, 32).map((check) => ({
      ...
      required: check?.required === true, detail: String(check?.detail || '').slice(0, 240),
    })),
  };
}
...
function safeDraftResult(result = {}) {
  ...
  if (!/^[A-Za-z0-9_-]+$/.test(postId)) throw new Error('网易号草稿回读结果缺少有效草稿 ID');
  let url;
  try { url = new URL(String(result.postUrl || '')); } catch { throw new Error('网易号草稿回读结果缺少有效草稿 URL'); }
  const expectedHash = `#/article-publish/${encodeURIComponent(postId)}`;
  if (url.protocol !== 'https:' || url.hostname !== 'mp.163.com'
      || url.pathname !== '/subscribe_v4/index.html' || url.hash !== expectedHash) {
    throw new Error('网易号草稿 URL 不受信任');
  }
  if (result.success !== true || result.draftOnly !== true || result.readBackVerified !== true || result.fidelityVerified !== true) {
    throw new Error('网易号草稿未通过保存后 HTML 内容保真校验，不能标记 draft_saved');
  }
  const fidelity = sanitizeFidelityReport(result.fidelityReport);
  if (!fidelity || fidelity.fidelityVerified !== true || !['PASS', 'DEGRADED'].includes(fidelity.overall)
      || fidelity.checks.some((check) => check.required === true && check.status !== 'PASS')) {
    throw new Error('网易号草稿保真报告缺失或必需检查未通过，不能标记 draft_saved');
  }
...
export class NeteaseDraftService {
  ...
  async prepare({ packageId, userConfirmed }) {
    ...
    const baseTaskKey = makeTaskKey({ packageId, platform: 'netease', accountId: ACCOUNT_ID, contentVersion: snapshot.source.contentVersion });
    ...
    const created = await this.store.createTask({
      ...
      platform: 'netease', platformName: '网易号', accountId: ACCOUNT_ID, accountLabel: '当前 Chrome 网易号会话',
    ...
    if (previous) {
      ...
      created.task.retryReason = '前次失败发生在网易号草稿保存请求之前；保留原审计记录后由用户重新确认重试';
    ...
    const task = await this.store.transitionStatus(created.task.taskId, TASK_STATUS.READY, '快照已锁定，等待扩展检查当前网易号登录会话');
    return { started: true, task, gate: snapshot.gate };
  }

  async begin({ taskId, snapshotId, userConfirmed }) {
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== MODE) throw new Error('找不到网易号草稿任务');
    ...
    const gate = checkRealActionGate({ action: 'saveDraft', platform: 'netease', authorization: {
      stage: '6-netease-draft', userConfirmed, snapshotVerified: true,
    } });
    if (!gate.allowed) throw new Error(gate.reason);
    const running = await this.store.transitionStatus(taskId, TASK_STATUS.RUNNING, '安全闸门已通过；仅允许保存网易号草稿');
    return { task: running, authorization: { action: 'saveDraft', platform: 'netease', snapshotId: task.snapshotId } };
  }

  async progress({ taskId, status, detail = '' }) {
    if (!PROGRESS.has(status)) throw new Error('不允许的网易号草稿进度状态');
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== MODE) throw new Error('找不到网易号草稿任务');
  ...
  async complete({ taskId, result }) {
    ...
    await this.store.transitionStatus(taskId, TASK_STATUS.DRAFT_SAVED, '网易号草稿已保存并通过 HTML 内容保真回读校验', { draftResult });
  ...
  async fail({ taskId, error, fidelityReport }) {
    ...
    return this.store.transitionStatus(taskId, TASK_STATUS.FAILED, String(error || '网易号草稿保存失败').slice(0, 500), fidelityFailure ? { fidelityFailure } : {});
  }
}
