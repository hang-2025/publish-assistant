...
function safeDraftResult(result = {}) {
  ...
  if (result.success !== true || result.draftOnly !== true || result.readBackVerified !== true) {
    throw new Error('知乎草稿未通过保存后回读校验，不能标记 draft_saved');
  }
  return { postId, postUrl: url.toString(), readBackVerified: true, savedAt: new Date().toISOString() };
}
...
export class ZhihuDraftService {
  ...
  async complete({ taskId, result }) {
    ...
    await this.store.transitionStatus(taskId, TASK_STATUS.DRAFT_SAVED, '知乎草稿已保存并通过回读校验', { draftResult });
  ...
  async fail({ taskId, error }) {
    ...
    return this.store.transitionStatus(taskId, TASK_STATUS.FAILED, String(error || '知乎草稿保存失败').slice(0, 500));
  }
}
