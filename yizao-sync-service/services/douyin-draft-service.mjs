...
const ACCOUNT_ID = 'douyin-current-session';
...
function safeDraftResult(result = {}) {
  ...
  if (!/^[0-9]+$/.test(postId)) throw new Error('抖音草稿回读结果缺少有效文章 ID');
  let url;
  try { url = new URL(String(result.postUrl || '')); } catch { throw new Error('抖音草稿回读结果缺少有效草稿 URL'); }
  if (url.protocol !== 'https:' || url.hostname !== 'creator.douyin.com'
      || !url.pathname.startsWith('/creator-micro/')) {
    throw new Error('抖音草稿 URL 不受信任');
  }
  if (result.success !== true || result.draftOnly !== true || result.readBackVerified !== true || result.fidelityVerified !== true) {
    throw new Error('抖音草稿未通过保存后 HTML 内容保真校验，不能标记 draft_saved');
  }
  ...
  if (!fidelity || fidelity.fidelityVerified !== true
    ...
    throw new Error('抖音草稿保真报告缺失或必需检查未通过，不能标记 draft_saved');
  }
  ...
  for (const key of REQUIRED_FIDELITY_CHECKS) {
    ...
    if (!check || check.required !== true || check.status !== 'PASS') {
      throw new Error(`抖音草稿保真报告缺少必需 PASS：${key}，不能标记 draft_saved`);
    }
  }
  return { postId, postUrl: url.toString(), readBackVerified: true, fidelityVerified: true, fidelity, savedAt: new Date().toISOString() };
}

/** Durable, single-platform coordinator. It never receives cookies and never calls Douyin itself. */
export class DouyinDraftService {
  ...
  async prepare({ packageId, userConfirmed }) {
    if (userConfirmed !== true) throw new Error('保存抖音草稿需要用户在当前操作中明确确认');
    ...
    const baseTaskKey = makeTaskKey({ packageId, platform: 'douyin', accountId: ACCOUNT_ID, contentVersion: snapshot.source.contentVersion });
    ...
    const created = await this.store.createTask({
      ...
      platform: 'douyin', platformName: '抖音', accountId: ACCOUNT_ID, accountLabel: '当前 Chrome 抖音会话',
      contentVersion: snapshot.source.contentVersion, title: snapshot.content.title,
      segments: context.segments, snapshotId: snapshot.snapshotId, snapshot: snapshotRecord(snapshot), mode: 'douyin-draft',
    });
    ...
    const task = await this.store.transitionStatus(created.task.taskId, TASK_STATUS.READY, '快照已锁定，等待扩展检查当前 抖音登录会话');
    return { started: true, task, gate: snapshot.gate };
  }

  async begin({ taskId, snapshotId, userConfirmed }) {
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== 'douyin-draft') throw new Error('找不到抖音草稿任务');
    ...
    const gate = checkRealActionGate({
      action: 'saveDraft', platform: 'douyin',
      authorization: { stage: '3-douyin-draft', userConfirmed, snapshotVerified: true },
    });
    if (!gate.allowed) throw new Error(gate.reason);
    const running = await this.store.transitionStatus(taskId, TASK_STATUS.RUNNING, '安全闸门已通过；仅允许保存抖音草稿');
    return { task: running, authorization: { action: 'saveDraft', platform: 'douyin', snapshotId: task.snapshotId } };
  }

  async progress({ taskId, status, detail = '' }) {
    if (!PROGRESS.has(status)) throw new Error('不允许的 抖音草稿进度状态');
    const task = await this.store.getTask(taskId);
    if (!task || task.mode !== 'douyin-draft') throw new Error('找不到抖音草稿任务');
  ...
  async complete({ taskId, result }) {
    ...
    await this.store.transitionStatus(taskId, TASK_STATUS.DRAFT_SAVED, '抖音草稿已保存并通过 HTML 内容保真回读校验', { draftResult });
  ...
  async fail({ taskId, error, fidelityReport }) {
    ...
    return this.store.transitionStatus(taskId, TASK_STATUS.FAILED, String(error || '抖音草稿保存失败').slice(0, 500), fidelityFailure ? { fidelityFailure } : {});
  }
}
