...
async function cmdSimulateOfficialTask(payload) {
  ...
  const start = await startSimulatedPublish({ store, snapshot, siteKey, lockDir: LOCK_DIR });
...
async function cmdGetTasks() {
  const tasks = (await store.listTasks()).map(sanitizeTask).filter(Boolean);
...
async function cmdGetTask(payload) {
  ...
  const task = sanitizeTask(await store.getTask(taskId));
...
async function cmdRemoveTask(payload) {
  ...
  const taskId = payload?.taskId;
  if (!TASK_ID_RE.test(String(taskId || ''))) throw new Error('taskId 格式无效');
  const result = await store.removeTask(taskId);
...
async function cmdPreviewExcelRegistration(payload) {
  ...
  const workbook = readXlsx(await fs.readFile(excelPath));
...
function isTrustedOrigin(req) {
  ...
  if (/^chrome-extension:\/\/[a-p]{32}$/i.test(origin)) return { ok: true, origin };
...
const server = http.createServer(async (req, res) => {
  const log = (status) => console.log(`[${new Date().toISOString()}] ${req.method} ${req.url} -> ${status}`);
  try {
    if (!isTrustedHost(req)) { send(res, 403, { error: '拒绝：Host 不受信任' }); log(403); return; }

    // 预检（扩展 fetch 必然带 Origin）
    if (req.method === 'OPTIONS') {
      const { ok, origin } = isTrustedOrigin(req);
    ...
    if (req.method === 'GET' && req.url === '/api/health') {
      send(res, 200, { ok: true, name: 'yizao-sync-service', version: VERSION, protocol: PROTOCOL });
      log(200);
      return;
    }

    if (req.method === 'POST' && req.url === '/api/command') {
      const { ok: originOk, origin } = isTrustedOrigin(req);
      if (!originOk) { send(res, 403, { error: '拒绝：仅允许插件内部页面调用（Origin 不受信任）' }); log(403); return; }

      const auth = (req.headers.authorization || '');
      const tokenMatch = auth.match(/^Bearer\s+([0-9a-f]{64})$/i);
      if (!tokenMatch || !crypto.timingSafeEqual(Buffer.from(tokenMatch[1], 'hex'), Buffer.from(STATE.token, 'hex'))) {
      ...
      for await (const chunk of req) {
        ...
        if (bodyBytes > MAX_BODY_BYTES) { send(res, 413, { error: '请求体过大' }, origin); log(413); req.destroy(); return; }
        body += chunk;
      }
      let message;
      try { message = JSON.parse(body || '{}'); } catch { send(res, 400, { error: '请求体不是有效 JSON' }, origin); log(400); return; }

      const handler = COMMANDS[message.command];
      if (!handler) { send(res, 400, { error: `未知命令：${String(message.command).slice(0, 40)}（白名单：${Object.keys(COMMANDS).join(', ')}）` }, origin); log(400); return; }
