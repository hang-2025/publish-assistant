...
保留阶段 1A~2J 的只读/模拟能力，并新增受保护的知乎单篇保存草稿协调。公开发布、独立真实上传、Excel 写入、文件移动/删除与真实归档仍没有命令；服务不会保存 Chrome Cookie/Profile，也不会修改文章或 Excel。
...
测试全部使用临时夹具或模拟 HTTP 响应，除既有覆盖外还验证：知乎未登录、公开发布拒绝、重复任务、快照变化、图片/保存/回读失败、非法状态转换和重启不自动重试。自动测试不会访问真实知乎账号。
